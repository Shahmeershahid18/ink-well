...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-[var(--card)] shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-250 flex flex-col',
  {
    variants: {
      side: {
...
>(({ side = 'right', className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ref={ref}
      className={cn(sheetVariants({ side }), 'border-[var(--border)]', className)}
      {...props}
    >
      {children}
...
const SheetFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      'flex flex-row justify-end gap-2 border-t border-[var(--border)] p-4',
      className,
    )}
    {...props}
  />
